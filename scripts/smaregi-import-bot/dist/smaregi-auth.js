...
export class SmaregiPlatformAuth {
    ...
    async getAccessToken() {
        if (!this.tokenData) {
            throw new Error("No token data available. Please run the setup command first to authorize the app.");
        }
     ...
     */
    extractContractIdFromJwt(token) {
        try {
    ...
    saveTokenToFile() {
        if (!this.tokenData)
            return;
        fs.writeFileSync(this.tokenFilePath, JSON.stringify(this.tokenData, null, 2), "utf-8");
 ...
 */
export function createSmaregiAuthFromEnv() {
    const clientId = process.env.SMAREGI_CLIENT_ID;
    const clientSecret = process.env.SMAREGI_CLIENT_SECRET;
    const redirectUri = process.env.SMAREGI_REDIRECT_URI || "http://localhost:3000/callback";
    // SMAREGI_USE_SANDBOX=true で開発環境を使用
    const useSandbox = process.env.SMAREGI_USE_SANDBOX === "true";
    if (!clientId || !clientSecret) {
        throw new Error("Missing required environment variables: SMAREGI_CLIENT_ID, SMAREGI_CLIENT_SECRET");
    }
